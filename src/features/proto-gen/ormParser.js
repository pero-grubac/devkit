...
function toProtoType(raw) {
  ...
  if (t.includes("float") || t.includes("numeric") || t.includes("decimal"))
    return "float";
  if (t.includes("double")) return "double";
  if (t.includes("boolean") || t.includes("bool")) return "bool";
  if (t.includes("datetime") || t.includes("timestamp") || t.includes("date"))
    return "google.protobuf.Timestamp";
  if (t.includes("json") || t.includes("jsonb")) return "google.protobuf.Struct";
  ...
  if (
    ...
    return "string";
  if (t.includes("bytes") || t.includes("blob") || t.includes("binary"))
    return "bytes";
...
export function parseSqlAlchemy(src) {
  ...
  while ((cm = classRe.exec(src)) !== null) {
    ...
    const block =
      nextCm > 0 ? src.slice(start, start + nextCm) : src.slice(start);
    const fields = [];

    // Column fields:  name = Column(Type, ...)
    const colRe = /(\w+)\s*=\s*Column\s*\(([^)]+(?:\([^)]*\)[^)]*)*)\)/g;
    let fr;
    while ((fr = colRe.exec(block)) !== null) {
      const fieldName = fr[1];
      if (fieldName.startsWith("__")) continue;
      const colArgs = fr[2].trim();
      const isOpt = colArgs.includes("nullable=True");

      // ForeignKey — treat as int64 reference, rename to field_id
      if (colArgs.includes("ForeignKey")) {
        const fkName = fieldName.endsWith("_id")
          ? fieldName
          : fieldName + "_id";
        fields.push({ name: fkName, rawType: "int64", optional: isOpt });
        continue;
      }

      // Normal Column — extract first arg as type: String(64) → String
      const firstArg = colArgs.split(",")[0].trim();
      const innerType = firstArg.replace(/\s*\(.*$/, "").trim();
...
export function parsePydantic(src) {
  ...
  const classRe =
    /class\s+(\w+)\s*\((?:BaseModel|BaseSettings|RootModel|SQLModel)[^)]*\)\s*:/g;
  const fieldRe =
    /^\s{4}(\w+)\s*:\s*([^\s=#\n]+)(?:\s*=\s*(?:Field|field)?\s*[^#\n]*)?/gm;
  let cm;
  while ((cm = classRe.exec(src)) !== null) {
    ...
    while ((fr = fieldRe.exec(block)) !== null) {
      let rawType = fr[2];
      const isOpt =
        rawType.startsWith("Optional[") || rawType.includes("| None");
      rawType = rawType
        .replace(/Optional\[(.+)\]/, "$1")
        .replace(/\s*\|\s*None/, "")
        .replace(/List\[(.+)\]/, "List:$1")
...
export function parseEntityFramework(src) {
  ...
  while ((cm = classRe.exec(src)) !== null) {
    const name = cm[1];
    if (
      ["DbContext", "ApplicationDbContext", "IdentityUser"].some((s) =>
        name.includes(s),
      )
    )
      continue;
    ...
    const processProps = (re) => {
      ...
      while ((fr = re.exec(block)) !== null) {
        ...
        const collMatch = rawType.match(
          /^(?:ICollection|List|IEnumerable|IList|HashSet)<(\w+)>$/,
        );
        if (collMatch) {
          fields.push({
            name: fieldName,
        ...
        if (fieldName !== "Id" || fields.length === 0)
          // avoid duplicate Id
          fields.push({ name: fieldName, rawType: protoType, optional: isOpt });
      }
    };
...
export function parseJpa(src) {
  ...
  while ((cm = classRe.exec(src)) !== null) {
    ...
    while ((fr = fieldRe.exec(block)) !== null) {
      ...
      const collMatch = rawType.match(
        /^(?:List|Set|Collection|ArrayList|LinkedList)<(\w+)>$/,
      );
      if (collMatch) {
...
export function parseTypeOrm(src) {
  ...
  while ((cm = classRe.exec(src)) !== null) {
    ...
    const colRe =
      /(?:@(?:PrimaryGeneratedColumn|PrimaryColumn|Column|CreateDateColumn|UpdateDateColumn|DeleteDateColumn)\([^)]*\)\s*)+(\w+)\s*[?!]?\s*:\s*([\w<>|\[\] ]+)/g;
    let fr;
    while ((fr = colRe.exec(block)) !== null) {
      const fName = fr[1];
      let rawType = fr[2]
      ...
      const arrMatch =
        rawType.match(/^(\w+)\[\]$/) || rawType.match(/^Array<(\w+)>$/);
      if (arrMatch) {
    ...
    const relRe =
      /@(?:OneToMany|ManyToMany)\([^)]*\)\s+(\w+)\s*[?!]?\s*:\s*([\w<>]+)/g;
    let rr;
    while ((rr = relRe.exec(block)) !== null) {
      const inner = rr[2]
        .replace(/^(?:Promise|Relation)<(.+)>$/, "$1")
        .replace(/\[\]$/, "");
    ...
    const relOneRe =
      /@(?:ManyToOne|OneToOne)\([^)]*\)\s+(\w+)\s*[?!]?\s*:\s*(\w+)/g;
    let ro;
...
const SQLALCHEMY_SAMPLE = `from sqlalchemy import Column, Integer, String, Float, DateTime, Boolean, ForeignKey
...
export const ORM_MAP = {
  sqlalchemy: {
    label: "SQLAlchemy",
    group: "Python",
    parser: parseSqlAlchemy,
    placeholder: SQLALCHEMY_SAMPLE,
  },
  django: {
    label: "Django ORM",
    group: "Python",
    parser: parseDjangoOrm,
    placeholder: DJANGO_SAMPLE,
  },
  pydantic: {
    label: "Pydantic",
    group: "Python",
    parser: parsePydantic,
    placeholder: PYDANTIC_SAMPLE,
  },
  ef: {
    label: "Entity Framework",
    group: "C#",
    parser: parseEntityFramework,
    placeholder: EF_SAMPLE,
  },
  jpa: {
    label: "JPA/Hibernate",
    group: "Java",
    parser: parseJpa,
    placeholder: JPA_SAMPLE,
  },
  typeorm: {
    label: "TypeORM",
    group: "TypeScript",
    parser: parseTypeOrm,
    placeholder: TYPEORM_SAMPLE,
  },
  prisma: {
    label: "Prisma",
    group: "TypeScript",
    parser: parsePrisma,
    placeholder: PRISMA_SAMPLE,
  },
};
