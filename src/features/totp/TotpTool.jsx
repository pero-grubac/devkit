...
export function TotpTool() {
  ...
  const lastCounterRef = useRef(Math.floor(Date.now() / 1000 / 30));
  useEffect(() => {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      // Refresh codes whenever the 30s window actually changed — comparing against
      // the last known counter (instead of requiring `remaining === 30` to land
      // exactly) survives setInterval drift/tab-throttling, which otherwise causes
      // the exact boundary tick to be skipped and the code to freeze forever.
      const counter = Math.floor(epoch / 30);
      if (counter !== lastCounterRef.current) {
        lastCounterRef.current = counter;
        computeCodes(secretRef.current);
      }
    };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <Card>
        ...
        >
          Paste the Base32 secret from any TOTP QR code (Google Authenticator,
          Authy, 1Password, etc).
          <br />
          Codes refresh every 30 seconds. Compatible with RFC 6238 / otpauth://
          URIs.
        </div>
      </Card>
    </div>
